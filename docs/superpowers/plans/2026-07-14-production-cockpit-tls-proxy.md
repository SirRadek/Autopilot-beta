...
Provision the trusted bootstrap boundary only through an independent VM-image
root channel, according to
`ops/cockpit-proxy/trusted-bootstrap-contract.json`. That channel installs the
reviewed launcher as root-owned mode `0755` and independently creates the
root-owned mode-`0400` authorization record. The record binds the canonical
checkout and origin, accepted SHA, `radek` UID/GID, every privileged payload
SHA-256, payload count, schema version, and a SHA-256 authorization ID over all
preceding record lines. Both artifacts must pre-exist before the unprivileged
operator invokes the launcher; neither may be generated by the candidate
checkout or by a command run with `sudo` from that checkout.
