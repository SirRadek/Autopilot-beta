...
//   tsx related-files-status-cli.ts --root <repo> [--prior snap.json] [--snapshot-out snap.json]
//
...
//
// Read-only; `--root` may point at any repo with a mesh/nodes dir (e.g. the canonical
// autopilot mesh) without mutating it.
...
if (writeBaseline !== undefined) {
  const report = computeRelatedFilesStatus(root);
