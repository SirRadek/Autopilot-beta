...
export function ApprovalPane({ approvals, selectedApprovalId, onSelect, onApprove, onReject, busyApprovalId, error, auditResult }: ApprovalPaneProps) {
  ...
  return <div className="approval-pane"><div className="approval-queue-header"><span>{sorted.length} approval{sorted.length === 1 ? "" : "s"}</span><span>{sorted.filter((item) => item.status === "pending").length} pending</span></div>
    {sorted.length === 0 ? <p className="approval-empty">No approval requests.</p> : <ul className="approval-queue" aria-label="Approval queue">{sorted.map((approval) => <li key={approval.approval_id}><button type="button" className={approval.approval_id === selected?.approval_id ? "approval-row selected" : "approval-row"} aria-current={approval.approval_id === selected?.approval_id ? "true" : undefined} onClick={() => select(approval)}><span className="approval-row-title">{approval.vendor}{approval.model ? ` / ${approval.model}` : ""}</span><span className={`approval-row-status approval-status-${approval.status}`}>{approval.status}</span><span className="approval-row-preview">{boundedPromptPreview(approval.prompt_preview, 120)}</span><span className="approval-row-tokens">{approval.estimated_tokens.toLocaleString()} tokens</span></button></li>)}</ul>}
