...
describe("RunDetailView", () => {
  ...
  it("uses instance-scoped section ids", () => { const host = document.createElement("div"); document.body.append(host); const root = createRoot(host); act(() => root.render(<><RunDetailView run={baseRun} runInspector={null} promotionPane={null} incidentPane={null} /><RunDetailView run={baseRun} runInspector={null} promotionPane={null} incidentPane={null} /></>)); const ids = [...host.querySelectorAll("[id]")].map((node) => node.id); expect(new Set(ids).size).toBe(ids.length); act(() => root.unmount()); host.remove(); });
});
