...
describe("RunDetailView", () => {
  ...
  it("keeps pause planned and disables cancel without a handler", () => { const { host, root } = mount(baseRun); const buttons = [...host.querySelectorAll(".run-status-actions button")]; expect(buttons.map((button) => button.textContent)).toEqual(["Pauza", "Zastavit"]); expect((buttons[0] as HTMLButtonElement).disabled).toBe(true); expect(buttons[0]?.getAttribute("title")).toBe("Planned"); expect((buttons[1] as HTMLButtonElement).disabled).toBe(true); expect(buttons[1]?.getAttribute("title")).toBeNull(); expect(host.querySelectorAll(".planned-badge")).toHaveLength(1); act(() => root.unmount()); host.remove(); });
