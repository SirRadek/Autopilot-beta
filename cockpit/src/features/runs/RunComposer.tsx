...
export function RunComposer({ projects, quotas, models, onPrepare, onApprove }: RunComposerProps) {
  ...
  const canPrepare = selectedProject !== undefined && prompt.trim() !== "" && promptTokens < 9_000 && (promptTokens <= 1_000 || promptReviewAcknowledged) && provider !== "" && routeFresh && selectedModel !== "" && !pendingPrepareKeys.has(boundKey);
