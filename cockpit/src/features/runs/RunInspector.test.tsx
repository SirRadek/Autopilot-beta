...
const baseRun = {
  ...
  current: { run_id: "run-1", revision: 1, project_id: "project-1", prompt: "Exact approved prompt", provider: "codex_cli", model: "gpt-5", input_token_bound: 21, output_token_allowance: 8_192, estimated_tokens: 8_213, requested_artifacts: ["text", "visual"], created_at: "2026-07-13T10:00:00Z" },
  revisions: [{ run_id: "run-1", revision: 1, project_id: "project-1", prompt: "Exact approved prompt", provider: "codex_cli", model: "gpt-5", input_token_bound: 21, output_token_allowance: 8_192, estimated_tokens: 8_213, requested_artifacts: ["text", "visual"], created_at: "2026-07-13T10:00:00Z" }], supervisor_task_id: "task-1", worker_run_id: "worker-1", terminal_reason: null,
  ...
  reservation_status: "settled", provider_result: { refused: false, reason: null, worker_run_id: "worker-1", raw_output: "x".repeat(5000), exit_code: 0, error_reason: null, lock_status: "acquired_supervisor_spawn" },
...
describe("RunInspector", () => {
  ...
  it("never substitutes current input when the approved revision is unavailable", () => { const { host, root } = mount({ ...baseRun, approved_revision: 2, current: { ...baseRun.current, revision: 2, prompt: "UNAPPROVED CURRENT INPUT" } }); expect(host.textContent).toContain("Schválená revize 2 není dostupná"); expect(host.textContent).not.toContain("UNAPPROVED CURRENT INPUT"); expect(host.querySelector('[role="alert"]')).not.toBeNull(); act(() => root.unmount()); host.remove(); });
  it("caps artifact previews with an explicit marker", () => { const { host, root } = mount({ ...baseRun, artifacts: [{ ...baseRun.artifacts[0]!, preview: `preview-${"z".repeat(3000)}-tail` }] }); expect(host.textContent).toContain("Náhled artefaktu byl zkrácen"); expect(host.textContent).not.toContain("-tail"); act(() => root.unmount()); host.remove(); });
  it("uses instance-scoped section ids", () => { const host = document.createElement("div"); document.body.append(host); const root = createRoot(host); act(() => root.render(<><RunInspector run={baseRun} timeline={timeline} /><RunInspector run={baseRun} timeline={timeline} /></>)); const ids = [...host.querySelectorAll("[id]")].map((node) => node.id); expect(new Set(ids).size).toBe(ids.length); act(() => root.unmount()); host.remove(); });
  it("types actual Control Plane worker wire metadata and cumulative retry accounting", () => { const typed: RunRecord = { ...baseRun, retry_input_tokens: 10, retry_output_tokens: 20, provider_result: { refused: false, reason: null, worker_run_id: "worker-1", raw_output: "failed", exit_code: 7, error_reason: "provider_failed", lock_status: "failed" } }; expect(typed.provider_result?.exit_code).toBe(7); expect(typed.retry_input_tokens + typed.retry_output_tokens).toBe(30); });
});
