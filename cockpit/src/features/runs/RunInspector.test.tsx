...
const baseRun = {
  ...
  revisions: [{ run_id: "run-1", revision: 1, project_id: "project-1", prompt: "Exact approved prompt", provider: "codex_cli", model: "gpt-5", estimated_tokens: 25, requested_artifacts: ["text", "visual"], created_at: "2026-07-13T10:00:00Z" }], supervisor_task_id: "task-1", worker_run_id: "worker-1", terminal_reason: null,
...
describe("RunInspector", () => {
  ...
  it("types worker failure metadata and cumulative retry accounting", () => { const typed: RunRecord = { ...baseRun, retry_input_tokens: 10, retry_output_tokens: 20, provider_result: { refused: false, reason: null, worker_run_id: "worker-1", raw_output: "failed", exitCode: 7, errorReason: "provider_failed", lockStatus: "failed" } }; expect(typed.provider_result?.exitCode).toBe(7); expect(typed.retry_input_tokens + typed.retry_output_tokens).toBe(30); });
});
