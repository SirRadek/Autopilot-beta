...
export type ProbeProviderId = "codex_cli" | "claude_cli" | "agy_cli";
export interface ProbeRefreshResult { readonly accepted: readonly ProbeProviderId[]; readonly rejected: readonly ProbeProviderId[]; readonly expires_at: string }
export interface ProviderProbeLeaseState { readonly leased: boolean; readonly expires_at: string | null }
export type ProviderProbeFailurePhase = "launch" | "readiness" | "echo" | "render" | "cleanup";
export interface ProviderProbeFailure { readonly phase: ProviderProbeFailurePhase; readonly attempts?: number }
export type RunReasoningEffort = "low" | "medium" | "high" | "xhigh" | "max" | "ultra";
export interface ProviderModel { readonly model_id: string; readonly available: boolean; readonly health: string; readonly source: string; readonly reasoning_efforts?: readonly RunReasoningEffort[]; readonly fetched_at?: string | null; readonly updated_at?: string | null }
export interface ProviderQuota { readonly provider: string; readonly source: string; readonly fetched_at: string; readonly observed_at: string; readonly five_hour: ProviderQuotaWindow; readonly weekly: ProviderQuotaWindow; readonly api_spend: number | null; readonly currency: string | null; readonly models: readonly ProviderModel[]; readonly health: string; readonly error_code: string | null; readonly probe_failure?: ProviderProbeFailure; readonly freshness: "fresh" | "stale" | "unavailable"; readonly next_poll_at: string | null; readonly cli_version?: string | null }
export interface ProviderModelRoute { readonly provider: string; readonly reasoning_efforts: readonly RunReasoningEffort[]; readonly configured?: boolean; readonly observed?: boolean; readonly available?: boolean; readonly health?: readonly string[]; readonly discovery?: string; readonly source?: "live_snapshot" | "static_fallback" | "mixed" }
export interface ProviderModelCatalog { readonly provider: string; readonly discovery: "models_cache"; readonly fetched_at: string; readonly freshness: "fresh" | "stale"; readonly age_ms: number }
export interface ProviderModels { readonly freshness: string; readonly fetched_at: string | null; readonly next_poll_at: string | null; readonly catalogs: readonly ProviderModelCatalog[]; readonly models: readonly { readonly model_id: string; readonly providers: readonly string[]; readonly configured?: boolean; readonly observed?: boolean; readonly available: boolean; readonly health: readonly string[]; readonly source?: "live_snapshot" | "static_fallback" | "mixed"; readonly reasoning_efforts: readonly RunReasoningEffort[]; readonly provider_routes?: readonly ProviderModelRoute[]; readonly fetched_at?: string | null; readonly updated_at?: string | null }[] }
export interface ProviderHealth { readonly providers: readonly { readonly provider: string; readonly health: string; readonly freshness: string; readonly fetched_at: string; readonly next_poll_at: string | null; readonly cli_version?: string | null; readonly error_code: string | null; readonly probe_failure: ProviderProbeFailure | null; readonly lease: ProviderProbeLeaseState }[] }
...
export interface ProjectCreateInput { readonly name: string; readonly cwd: string; readonly enabled?: boolean }
