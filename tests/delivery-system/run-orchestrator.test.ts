...
function setup(options: { dispatch?: ReturnType<typeof vi.fn>; reserve?: ReturnType<typeof vi.fn>; enqueue?: ReturnType<typeof vi.fn>; projectRoot?: string; projectCwd?: string } = {}) {
  ...
  const input = { project_id: "alpha", prompt: "build it", provider: "codex_cli" as const, model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text", "visual"] as const, profile: "dev" as const, requested_reasoning_effort: null };
...
describe("governed run orchestration", () => {
  it("does not dispatch an out-of-root project when projectRoot is omitted", () => {
    ...
    try {
      ...
      expect(() => orchestrator.prepareRun({ project_id: "alpha", prompt: "blocked", provider: "codex_cli", model: null, estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null }))
  ...
  it("leaves the next unsafe recovered task queued until its project is corrected", async () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "recover safely", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("retries a failed worker result on the same route before succeeding", async () => {
    ...
    const draft = orchestrator.prepareRun({ project_id: "alpha", prompt: "build it", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    orchestrator.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("terminalizes worker failure only after retry exhaustion", async () => {
    ...
    const draft = orchestrator.prepareRun({ project_id: "alpha", prompt: "fail", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    orchestrator.approveAndQueueRun(draft.current.run_id, 1, "owner");
    expect(await orchestrator.runSupervisorOnce()).toBeNull();
  ...
  it("stops retries when cumulative actual usage exhausts the approved budget", async () => {
    ...
    const draft = orchestrator.prepareRun({ project_id: "alpha", prompt: "x", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    orchestrator.approveAndQueueRun(draft.current.run_id, 1, "owner");
    expect((await orchestrator.runSupervisorOnce())?.status).toBe("failed");
    expect(dispatch).toHaveBeenCalledTimes(1);
  ...
  it("reconstructs the durable route and completes after restart without reserving twice", async () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "restart me", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("cancels durable supervisor work and releases after restart exactly once", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "cancel me", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("does not redispatch when settlement succeeded before a persistence fault", async () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "once", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("reuses the reservation when reserve committed before an injected throw", () => {
    ...
    const draft = orchestrator.prepareRun({ project_id: "alpha", prompt: "reserve once", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("finds and releases an orphan reservation after reserve commit and restart cancellation", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "orphan reserve", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("cancels a task when enqueue committed before an injected throw", () => {
    ...
    const draft = orchestrator.prepareRun({ project_id: "alpha", prompt: "enqueue once", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("finishes cancellation after release committed before an injected throw", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "cancel fault", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("reconciles when durable bind commits before an injected throw", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "bind fault", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("treats queued transition commit-then-throw as queued without compensating", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "queue fault", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("persists cancellation intent and releases even when supervisor cancellation throws", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "cancel throw", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("replays success after the settled marker commits and bounds oversized output", async () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "large", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("replays refusal after release marker and supervisor cancel commit faults", async () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "refuse", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("replays enqueue compensation after release commits then throws", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "compensate", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("recovers when compensation clear commits before throwing", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "clear", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    expect(() => first.approveAndQueueRun(draft.current.run_id, 1, "owner")).toThrow("after_enqueue_commit");
  ...
  for (const phase of ["artifact", "finalized"] as const) {
    it(`does not redispatch when ${phase} persistence commits before throwing`, async () => {
      ...
      const draft = first.prepareRun({ project_id: "alpha", prompt: phase, provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
      first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("does not redispatch when supervisor completion commits before throwing", async () => {
    ...
    const first = createRunOrchestrator({ stateDir, projectRoot: fixtureProjectRoot, tokenGateway: new TokenGateway({ stateDir }), supervisor: throwingSupervisor, dispatch, now: () => now, isRouteAvailable: () => true });
    const draft = first.prepareRun({ project_id: "alpha", prompt: "complete", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    first.approveAndQueueRun(draft.current.run_id, 1, "owner");
  ...
  it("replays no-task enqueue compensation when release commits before throwing", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "no task", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
  ...
  it("replays no-task enqueue compensation when clear commits before throwing", () => {
    ...
    const draft = first.prepareRun({ project_id: "alpha", prompt: "no task clear", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
    expect(() => first.approveAndQueueRun(draft.current.run_id, 1, "owner")).toThrow("enqueue_before_commit");
    expect(readRunStore(stateDir).runs[0]).toMatchObject({ queue_compensation_requested: false, supervisor_task_id: null, reservation_status: "none" });
    const restarted = createRunOrchestrator({ stateDir, projectRoot: fixtureProjectRoot, tokenGateway: new TokenGateway({ stateDir }), supervisor: new SupervisorQueue({ stateDir }), dispatch: vi.fn(), now: () => now, isRouteAvailable: () => true });
    expect(restarted.approveAndQueueRun(draft.current.run_id, 1, "owner").status).toBe("queued");
  ...
  for (const phase of ["release", "reservation_terminal", "finalized"] as const) {
    it(`replays terminal dispatch failure after ${phase} commit fault`, async () => {
      ...
      const draft = first.prepareRun({ project_id: "alpha", prompt: "fail", provider: "codex_cli", model: "gpt-5", estimated_tokens: 20_000, requested_artifacts: ["text"], profile: "dev", requested_reasoning_effort: null });
      first.approveAndQueueRun(draft.current.run_id, 1, "owner");
